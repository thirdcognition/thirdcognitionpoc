...
def gen_subject(
    ...
    step_items: JourneyStepList = llm_gen_steps(
        content, amount, journey_instructions, instructions
    )
    ...
    for i, step in enumerate(step_items.steps):
        ...
        new_step = llm_gen_step(
            step, rag_chain, content, journey_instructions, instructions
        )
    ...
    return SubjectModel(
        title=title, summary=summary, steps=steps, instructions=instructions, amount=amount
    )


def gen_journey_subject(journey: JourneyModel, subject: SubjectModel) -> SubjectModel:
    ...
    with st.status(f"Building subject"):
        subject = gen_subject(
            doc_chain,
            compressed,
            journey.instructions or "",
            subject.instructions or "",
            subject.step_amount or 5,
        )
...
def get_journey_gen(journey_name):
    ...
    ] if default_category[0] != None else []
    ...
    if (
        ...
        with st.spinner("Generating journey titles and summaries"):
            files = []
            for i, subject in enumerate(journey_details.subjects):
                title, summary = llm_gen_title_summary(
                    subject.steps
                )
...
def main():
    ...
    with st.container(border=True):
        ...
        if "creating_journey" in st.session_state and (journey_create == None or journey_create.complete == False):
