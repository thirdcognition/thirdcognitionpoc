...
class BaseParserChain(BaseChain):
    ...
    def __call__(
        ...
        async def arerun_parser(x):
            print_params("Rerun format parser", x)
            x["completion"] = x["completion"].content.strip()
            return await retry_parser.aparse_with_prompt(x["completion"], x["prompt_value"])
        ...
        self.chain = parser_chain | RunnableLambda(arerun_parser if self.async_mode else rerun_parser)
