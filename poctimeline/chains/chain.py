...
class Chain(BaseParserChain):
    def __init__(self, llm_id: str = "default", prompt: PromptFormatter = text_formatter, check_for_hallucinations: bool = False, **kwargs):
        super().__init__(prompt=prompt, llm=llms[llm_id], retry_llm=llms[llm_id] if llm_id == "json" else llms["instruct_detailed"], **kwargs)
        self.llm_id = llm_id
    ...
    def __call__(
        ...
        if self.check_for_hallucinations:
            self.hallunication_chain = BaseValidationChain(parent_chain=self.chain, prompt=self.prompt, validation_llm=llms["instruct_detailed"], retry_llm=self.retry_llm)
