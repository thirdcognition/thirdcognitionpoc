...
def get_journey_items(state:Union[JourneyCreationState, SubjectCreationState, StepCreationState], config) -> tuple[JourneyModel, SubjectModel, StepModel, JourneyTemplate, SubjectTemplate]:
    journey:JourneyModel = state["journey"] if "journey" in state else None,
    subject:SubjectModel = journey.subjects[config["configurable"]["subject_index"]] if journey is not None else None,
    step:StepModel = state["step"] if "step" in state else subject.steps[config["configurable"]["step_index"]] if subject is not None else None,
    ...
    return journey, subject, step, journey_template, subject_template
...
async def actions_build(state: StepCreationState, config: RunnableConfig) -> StepCreationState:
    ...
    prompt = subject.prompts.step_actions if subject is not None else subject_template.subject_prompts.step_actions

    action_amount = subject_template.action_amounts[config["configurable"]["step_index"]] if config["configurable"]["step_index"] < len(subject_template.action_amounts) else subject_template.default_action_amount
...
async def actions_structured_build(state: StepCreationState, config: RunnableConfig) -> StepCreationState:
    ...
    instructions = ( journey.instructions if journey is not None else journey_template.journey_instruction + "\n" + subject.instructions if subject is not None else  subject_template.subject_instruction).strip()
...
async def content_build(state: StepCreationState, config: RunnableConfig) -> StepCreationState:
    journey, subject, step, journey_template, subject_template = get_journey_items(state, config)
    prompt = subject.prompts.step_content if subject is not None else subject_template.subject_prompts.step_content
...
async def intro_build(state: StepCreationState, config: RunnableConfig) -> StepCreationState:
    journey, subject, step, journey_template, subject_template = get_journey_items(state, config)
    prompt = subject.prompts.step_intro if subject is not None else subject_template.subject_prompts.step_intro

    class_intro = await get_base_chain("journey_step_intro")((prompt.system, prompt.user)).ainvoke(
        {
            "context": state["content"],
            "journey_instructions": journey.instructions if journey is not None else journey_template.journey_instruction,
            "instructions": subject.instructions if subject is not None else subject_template.subject_instruction,
            "subject":state["subject_title"],
...
async def new_steps_build(state: SubjectCreationState, config: RunnableConfig) -> SubjectCreationState:
    journey, subject, step, journey_template, subject_template = get_journey_items(state, config)
    prompt = subject.prompts.steps if subject is not None else subject_template.subject_prompts.steps

    concepts = state["concepts"] if "concepts" in state else subject.concepts if subject is not None else None
    if concepts is None:
...
async def map_subject_steps(state: SubjectCreationState, config: RunnableConfig) -> List[RunnableConfig]:
    return [
        Send("subject_step_build",
            {
...
async def combine_mapped_steps(state: SubjectCreationState, config: RunnableConfig) -> SubjectCreationState:
    steps = sorted(state["steps"], key=lambda x: next((step.title for step in state["initial_steps"] if step.title == x.title), None))
    return {
        "steps_sorted": steps,
        "steps_done" : True,
    }

async def summary_build(state: SubjectCreationState, config: RunnableConfig) -> SubjectCreationState:
    journey, subject, step, journey_template, subject_template = get_journey_items(state, config)
    steps = state["steps_sorted"]

    summary = await llm_edit([f"Title: {step.title}\nSubject:\n{step.subject}\nSummary:\n{step.summary}\nContent:\n{step.content or ""}" for step in state["steps"]], "Summarize the following content into a description.", summarize=True)
    ...
    new_subject = SubjectModel(
        ...
        instructions=subject.instructions if subject else subject_template.subject_instruction,
        step_amount=subject.step_amount if subject else subject_template.step_amount,
        action_amount=subject.action_amount if subject else subject_template.default_action_amount,
    )
