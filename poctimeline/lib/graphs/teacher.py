...
def get_planner_chain(section: JourneyItem):
    id = f"{section.title}:{section.summary}"
    if id in _planner_chain:
        return _planner_chain[id]

    prompts:JourneyPrompts = section.prompts
    prompt = None
...
def prepare_planner_input(section_data: JourneyItem, task_data: JourneyItem, previous_tasks: List[TeachingTask], amount=5):

    section_description = textwrap.dedent(f"""
        Main section: {section_data.title}
        Detailed section: {task_data.title}
...
def init_state(state:TeachingState, config: RunnableConfig):
    ...
    sub_section_index:int= config["sub_section_index"]
    action_index:int=config["action_index"]
    init_journey_chat(config["journey_name"])
    journey:Dict[str, JourneyDataTable] = get_db_journey(journey_name)
    module = journey[section_index].children[sub_section_index].children[action_index]

    id = f"{journey_name}{DELIMITER}{section_index}{DELIMITER}{sub_section_index}{DELIMITER}{action_index}"
...
def introduce_class(state: TeachingState):
    section_data:JourneyItem = state["section_data"]
    messages = state["pre_class_messages"]
    messages = messages + [AIMessage(content=section_data.intro)]
...
# question loop
# 1. Send the question to the llm to determine the next module
...
# 3. If the response is not valid, analyse the response and determine the next module
